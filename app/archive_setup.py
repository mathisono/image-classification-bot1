...
def merge_index(active: sqlite3.Connection, source_database: str, archive_root: str, batch_size: int = 1000) -> dict[str, Any]:
    source_path = Path(source_database).expanduser().resolve()
    root = Path(archive_root).expanduser().resolve()
    source_archive_root = source_path.parent.parent.resolve()
    if not source_path.is_file():
        raise FileNotFoundError(str(source_path))
    if root != source_archive_root and root not in source_archive_root.parents:
        raise RuntimeError(f'nested index is outside selected archive: {source_path}')
...
def mount_smb(share: str, mount_point: str, username: str, password: str, domain: str, helper_path: str) -> dict[str, str]:
    ...
    env = os.environ.copy()
    env.update({'SMB_CREDENTIALS_FILE': str(credentials), 'SMB_READ_ONLY': 'false', 'SMB_SUDO_NONINTERACTIVE': 'true'})
    result = subprocess.run([helper_path, share, str(mount)], env=env, text=True, capture_output=True, timeout=45, check=False)
